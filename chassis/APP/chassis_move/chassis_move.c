...
{					
	...
	UI_theta=theta+(PI/2);
	if(UI_theta>2*PI) UI_theta-=2*PI;
 ...
 static void power_limitation_jugement(void)
{
	...
	if(power>max_power*2.f)CHASSIS_vPID_max=5000;
	else if(power<max_power*1.6f)CHASSIS_vPID_max=8000;
	if(buffer<BUFFER_MAX*0.25f) CHASSIS_vPID_max=1900;
...
static void Power_limit(void)//新的功率限制
{
  ...
  if((buffer>60&&buffer<=250))  fly_buffer_flag=1;  //飞坡增益触发
