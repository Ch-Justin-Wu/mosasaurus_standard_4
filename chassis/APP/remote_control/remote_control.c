...
uint8_t sbus_rx_buffer[36] = {0}; // rhn   新的接收信息定义
...
static float caculate_linear_speed(int width, int mid, int min, int max)
{
	...
	if (width >= (mid + 2)) // 中间消除波动
		speed = (float)((float)(width - (mid + 2)) / (float)(max - (mid + 2)) * NORMAL_LINEAR_SPEED);
	else if (width <= (mid - 2))
		speed = (float)((float)(width - (mid - 2)) / (float)((mid - 2) - min) * NORMAL_LINEAR_SPEED);
	else
		speed = 0;
	return speed;
}
...
static float caculate_rotational_speed(int width, int mid, int min, int max)
{
	float speed = 0;
	if (width >= (mid + 2)) // 中间消除波动
		speed = (10.0f * (width - (mid + 2)) / (max - (mid + 2)) * NORMAL_ROTATIONAL_SPEED);
	else if (width <= (mid - 2))
		speed = (10.0f * (width - (mid - 2)) / ((mid - 2) - min) * NORMAL_ROTATIONAL_SPEED);
	else
		speed = 0;
	return speed;
}
...
//{
//	chassis_center.chassis_control_order.vx_set=caculate_linear_speed(rc_ctrl.rc.ch[1],RC_MIDD,RC_MINN,RC_MAXX);
//
//	chassis_center.chassis_control_order.wz_set=caculate_rotational_speed(rc_ctrl.rc.ch[2],RC_MIDD,RC_MINN,RC_MAXX);
//
//
//
//	switch(rc_ctrl.rc.s[0])
//	{
//		case 1: {chassis_center.chassis_control_order.chassis_mode=CHASSIS_STATIC;} break;
//		case 3: {chassis_center.chassis_control_order.chassis_mode=CHASSIS_NO_FORCE;} break;
//		case 2: {chassis_center.chassis_control_order.chassis_mode=CHASSIS_NORMAL;} break;
//		default:break;
//	}
//
//
