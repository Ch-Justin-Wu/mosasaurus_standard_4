...
void BaseVel_To_WheelVel(float linear_x, float linear_y, float angular_z)
{
	...
	// 因为电机ID顺序原因导致的结果
	// 实际电机ID顺序应该为 顺时针左上开始(2 1 3 4) 每个电机自身都是以右手螺旋为正方向，解算结果的电机正方向为有正y分量的方向
	chassis_motor1.target_speed = (int)(Kinematics.wheel2.target_speed.rpm * M3508_REDUCTION_RATIO);
	chassis_motor2.target_speed = -(int)(Kinematics.wheel1.target_speed.rpm * M3508_REDUCTION_RATIO);
...
void Get_Base_Velocities(void)
{
	...
	// 轮子线速度转换为底盘中心三轴的速度
	Kinematics.actual_velocities.angular_z = (Kinematics.wheel1.actual_speed.linear_vel - Kinematics.wheel2.actual_speed.linear_vel - Kinematics.wheel3.actual_speed.linear_vel + Kinematics.wheel4.actual_speed.linear_vel) / (4.0f * (half_width + half_length));
	Kinematics.actual_velocities.linear_x = (-Kinematics.wheel1.actual_speed.linear_vel + Kinematics.wheel2.actual_speed.linear_vel - Kinematics.wheel3.actual_speed.linear_vel + Kinematics.wheel4.actual_speed.linear_vel) / (4.0f);
	Kinematics.actual_velocities.linear_y = (Kinematics.wheel1.actual_speed.linear_vel + Kinematics.wheel2.actual_speed.linear_vel + Kinematics.wheel3.actual_speed.linear_vel + Kinematics.wheel4.actual_speed.linear_vel) / (4.0f);
}
